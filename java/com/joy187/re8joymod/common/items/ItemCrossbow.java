...
public class ItemCrossbow extends ShootableItem implements IVanishable {

    public ItemCrossbow() {
        super(new ItemCrossbow.Properties().tab(RegistryEvents.RE8GROUP).stacksTo(1));
    }
...
//            ItemStack itemstack = playerentity.getProjectile(p_77615_1_);
            ...
            if (!itemstack.isEmpty() || flag) {
                if (itemstack.isEmpty()) {
                    itemstack = new ItemStack(ModItems.BOWSHOOT.get().asItem());
                }

                float f = getPowerForTime(i);
                if (!((double)f < 0.1D)) {
                    boolean flag1 = playerentity.abilities.instabuild || (itemstack.getItem() instanceof ItemBOWShoot && ((ItemBOWShoot)itemstack.getItem()).isInfinite(itemstack, p_77615_1_, playerentity));
                    if (!p_77615_2_.isClientSide) {
                        ItemBOWShoot arrowitem = (ItemBOWShoot)(itemstack.getItem() instanceof ItemBOWShoot ? itemstack.getItem() : ModItems.BOWSHOOT.get().asItem());
                        EntityBOWShoot abstractarrowentity = arrowitem.createArrow(p_77615_2_, itemstack, playerentity);

                        abstractarrowentity = customArrow(abstractarrowentity);
                        abstractarrowentity.setItem(itemstack);
                        abstractarrowentity.shootFromRotation(playerentity, playerentity.xRot, playerentity.yRot, 0.0F, f * 25.0F, 1.0F);
                        ...
                        abstractarrowentity.playSound(SoundEvents.ARROW_SHOOT, 2.0F,2.0F);
//                        if (f == 1.0F) {
...
//        }
    ...
    public  EntityBOWShoot customArrow(EntityBOWShoot arrow) {
        return arrow;
    }
    ...
    {
        if (this.isCrossbowB(player.getItemInHand(Hand.OFF_HAND)))
        {
            return player.getItemInHand(Hand.OFF_HAND);
        }
        else if (this.isCrossbowB(player.getItemInHand(Hand.MAIN_HAND)))
        {
    ...
    protected boolean isCrossbowB(ItemStack stack)
    {
        return stack.getItem() instanceof ItemBOWShoot;
    }
    ...
    public void appendHoverText(ItemStack stack, World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn){
        ...
        tooltip.add(ITextComponent.nullToEmpty(TextFormatting.GOLD+"Ammo Type:BOW Shoot"));
    }
    ...
    public void inventoryTick(ItemStack p_77663_1_, World p_77663_2_, Entity p_77663_3_, int p_77663_4_, boolean p_77663_5_) {
        if (p_77663_3_ instanceof PlayerEntity) {
            ...
            if (item.getItem() == ModItems.CROSSBOW1.get()) {
