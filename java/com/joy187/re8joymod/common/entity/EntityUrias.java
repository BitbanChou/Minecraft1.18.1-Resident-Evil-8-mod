...
public class EntityUrias extends RavagerEntity {


    public EntityUrias(EntityType<? extends RavagerEntity> type, World worldIn) {
        super(type, worldIn);
        this.xpReward = 70;
    ...
    public void aiStep() {
        ...
        else if(co==2)
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                float deltaOmega = 1.0f * i;//ModConfig.DEBUG_CONF.HALO_OMEGA;
                Vector3d pos = new Vector3d(this.position().x + 1 * Math.sin(0 + deltaOmega),  this.position().y + 0.1f * this.random.nextFloat(), this.position().z + 1 * Math.cos(0 + deltaOmega));
                this.level.addParticle(ParticleTypes.DRIPPING_LAVA, pos.x, pos.y, pos.z, 0,0,0);
            }
        }
        if (this.isAlive()) {
            if (this.isImmobile()) {
