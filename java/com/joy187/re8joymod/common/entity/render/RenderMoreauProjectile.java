...
public class RenderMoreauProjectile extends EntityRenderer<EntityMoreauProjectile> {


    public static final ResourceLocation TEXTURE = new ResourceLocation(Utils.MOD_ID ,"textures/item/zhihu.png");
    private static final RenderType field_229044_e_ = RenderType.entityCutoutNoCull(TEXTURE);

    public RenderMoreauProjectile(EntityRendererManager manager) {
        super(manager);
    }

    protected int getSkyLightLevel(EntityMoreauProjectile p_239381_1_, BlockPos p_239381_2_) {
        return 1;
    }


    public void render(EntityMoreauProjectile entityIn, float entityYaw, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn) {
...
//        matrixStackIn.(Vector3f.YP.rotationDegrees(180.0F));
    ...
    @Override
    public ResourceLocation getTextureLocation(EntityMoreauProjectile  p_110775_1_) {
        return TEXTURE;
    }
    ...
    public static class RenderFactory implements IRenderFactory<EntityMoreauProjectile> {

        @Override
        public EntityRenderer<? super EntityMoreauProjectile> createRenderFor(EntityRendererManager manager) {
            manager.setRenderShadow(false);
            return new RenderMoreauProjectile(manager);
        }
    }
