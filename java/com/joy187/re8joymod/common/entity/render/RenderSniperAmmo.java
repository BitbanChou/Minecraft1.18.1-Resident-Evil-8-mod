...
public class RenderSniperAmmo extends EntityRenderer<EntitySniperAmmo> {
    ...
    public RenderSniperAmmo(EntityRendererManager manager) {
        super(manager);
    }
    ...
    public void render(EntitySniperAmmo entityIn, float entityYaw, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn) {
...
//        matrixStackIn.(Vector3f.YP.rotationDegrees(180.0F));
    ...
    public static class RenderFactory implements IRenderFactory<EntitySniperAmmo> {

        @Override
        public EntityRenderer<? super EntitySniperAmmo> createRenderFor(EntityRendererManager manager) {
            manager.setRenderShadow(false);
            return new RenderSniperAmmo(manager);
        }
    }
