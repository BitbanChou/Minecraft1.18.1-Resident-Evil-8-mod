...
public class RenderPipeb extends EntityRenderer<EntityPipeb> {


    public static final ResourceLocation TEXTURE = new ResourceLocation(Utils.MOD_ID ,"textures/entity/pipebomb.png");
    private static final RenderType field_229044_e_ = RenderType.entityCutoutNoCull(TEXTURE);

    public RenderPipeb(EntityRendererManager manager) {
        super(manager);
    }

    protected int getSkyLightLevel(EntityPipeb p_239381_1_, BlockPos p_239381_2_) {
        return 5;
    }


    public void render(EntityPipeb entityIn, float entityYaw, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn) {
...
//        matrixStackIn.(Vector3f.YP.rotationDegrees(180.0F));
    ...
    @Override
    public ResourceLocation getTextureLocation(EntityPipeb  p_110775_1_) {
        return TEXTURE;
    }
    ...
    public static class RenderFactory implements IRenderFactory<EntityPipeb> {

        @Override
        public EntityRenderer<? super EntityPipeb> createRenderFor(EntityRendererManager manager) {
            manager.setRenderShadow(false);
            return new RenderPipeb(manager);
        }
    }
