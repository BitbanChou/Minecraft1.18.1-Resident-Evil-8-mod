...
public class RenderMoSpitter extends EntityRenderer<EntityMoSpitter> {
    ...
    public RenderMoSpitter(EntityRendererManager manager) {
        super(manager);
    }
    ...
    public void render(EntityMoSpitter entityIn, float entityYaw, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn) {
...
//        matrixStackIn.(Vector3f.YP.rotationDegrees(180.0F));
    ...
    public static class RenderFactory implements IRenderFactory<EntityMoSpitter> {

        @Override
        public EntityRenderer<? super EntityMoSpitter> createRenderFor(EntityRendererManager manager) {
            manager.setRenderShadow(false);
            return new RenderMoSpitter(manager);
        }
    }
